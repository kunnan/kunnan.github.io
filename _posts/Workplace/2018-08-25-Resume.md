...
- 使用的技术
  ...
  * 动态库的注入原理一个是基于修改[Mach-O ](https://zhangkn.github.io/2018/03/Hopper/) 的Load Commands，一个是DYLD_INSERT_LIBRARIES,例如使用它进行dumpdecrypted（补充：**Clutch** 通过`posix_spawnp`生成一个新的进程，然后暂停进程并dump内存）。
