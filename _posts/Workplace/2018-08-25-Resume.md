...
- 使用的技术
  * object-c 的运行时API： 动态新增属性(objc_setAssociatedObject、objc_getAssociatedObject)；修改和获取属性（class_getInstanceVariable、object_setIvar、object_getIvar）；swizzling交换替换方法的实现（class_getInstanceMethod、class_addMethod、class_replaceMethod、method_exchangeImplementations）
  * lldb、theos、monkeydev、lipo、cycript、hopper、[frida-ios-dump](https://github.com/zhangkn/frida-ios-dump)
