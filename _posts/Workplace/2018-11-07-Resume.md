...
>
>   - 1） 消息推送机制、KVC、KVO、GCD、block、CocoaPod、IAP内购、runtime、lldb、UIBackgroundModes、NSPredicate； 
...
>   - 4） [foundation框架（OC语言）、core foundation 框架（C语言,例如通讯录就是基于这个框架）](http://blog.csdn.net/z929118967/article/details/74332012)
>
...
>
>   - 1）dispatch：dispatch_after、dispatch_async、dispatch_sync、dispatch_barrier_async（ 场景：在写的过程中不能被读：要等到当前所有并发的block都执行完毕，才会单独执行这个barrier block代码块，等到这个barrier block执行完毕，再继续正常处理其他并发block） 
>
...
>
>   - 3) dispatch信号量（dispatch semaphore）: 常用来保证是block同步的执行
>
