...
* 负责新版的全城淘 app的整体功能开发：
  * 支持中英文版本切换
  * 新增注册模块、防止crash框架；适配最新系统API，提高app整体的操作流畅度和UI用户体验;
  * 负责蓝牙标签打印、票据打印模块； 负责有品的开单模块（购物车、SKU、下单、结算）、无品的收银台、收款信息报表模块（账单流水、收款报表、收银员报表、终端报表）、店铺模块（快捷订单、销售订单、会员、商品管理）。
* 负责CRM app的框架搭建，包括以下功能的开发维护：
  * 代理商系统：公海商户模块、待办事项模块、软件订单、订货单、终端绑定、终端维修、收费管理
